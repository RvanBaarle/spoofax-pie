...
public class TigerAnalyzeMulti extends ConstraintAnalyzeMultiTaskDef {
    ...
    protected MultiFileResult analyze(ExecContext context, ResourcePath root, HashMap<ResourceKey, IStrategoTerm> asts, ConstraintAnalyzerContext constraintAnalyzerContext) throws Exception {
        ...
        context.require(classLoaderResources.tryGetAsLocalResource(ConstraintAnalyzerFactory.class), ResourceStampers.hashFile());
        final StrategoRuntime strategoRuntime = context.require(getStrategoRuntimeProvider, None.instance).getValue().get();
        return constraintAnalyzer.analyze(root, asts, constraintAnalyzerContext, strategoRuntime);
    }
}
