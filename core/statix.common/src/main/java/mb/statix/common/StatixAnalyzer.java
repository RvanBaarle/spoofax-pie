...
public class StatixAnalyzer {
    ...
    public SolverContext createContext(@Nullable ITermVar focusVar) {
        return new SolverContext(spec.getSpec(), focusVar);
    }
    ...
    public SolverState createStartState(ITerm statixAst, String specName, String rootRuleName) {
        ...
        return SolverState.of(specSpec, State.of(specSpec), ImmutableList.of(rootConstraint));
    }
    ...
    public SolverState analyze(SolverContext ctx, SolverState startState) throws InterruptedException {
        final List<SolverState> states = InferStrategy.getInstance().eval(ctx, startState).toList();
