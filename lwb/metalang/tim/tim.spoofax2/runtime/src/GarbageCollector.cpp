...
std::optional<const StackRecord> GarbageCollector::find_record(uint64_t ret_addr) {
...
void GarbageCollector::scan_stack(void *fp) {
    ...
    if (!recordOpt.has_value()) {
        DEBUG_LOG("couldn't find record for current stack position");
        return;
    }
    auto &record = recordOpt.value();
    DEBUG_LOG("Found record: " << record.getInstructionOffset());
    uint32_t offset = 3 + record.getLocation(2).getSmallConstant();
    uint8_t **caller_sp;
#if defined(__x86_64__)
    caller_sp = static_cast<uint8_t **>(fp) + 2;
#define SP_DWARF_NUM 7
#elif defined(__aarch64__)
    caller_sp = static_cast<uint8_t **>(fp) + 2;
#define SP_DWARF_NUM 31
#else
#error Support for this architecture is not yet implemented
    ...
    for (uint32_t i = offset; i < record.getNumLocations(); i += 2) {
        ...
        assert(base_loc.getDwarfRegNum() == SP_DWARF_NUM);
        assert(der_loc.getDwarfRegNum() == SP_DWARF_NUM);
        ...
        DEBUG_LOG("\tBase pointer: " << (void *) base_ptr);
        DEBUG_LOG("\tDerived pointer: " << (void *) der_ptr);
        relocate(base_ptr);
        der_ptr = base_ptr + derived_offset;
        DEBUG_LOG("\tBase relocated to: " << (void *) base_ptr);
        DEBUG_LOG("\tDerived relocated to: " << (void *) der_ptr);
    }
}
...
void GarbageCollector::relocate(uint8_t *&pointer) {
    if (old_space.contains(pointer)) {
        ...
        DEBUG_LOG("\tRelocating " << (void *) base_pointer << " to " << (void *) new_pointer);
        base_pointer = new_pointer;
        pointer = base_pointer + offset;
        metadata->forwarded_pointer = new_pointer;
    } else {
        DEBUG_LOG("\tNot relocating " << (void *) pointer);
    }
}
    ...
    while (scan_ptr < space.free_ptr) {
        ...
        if (metadata->has_flags(BITFIELD_FLAG)) {
            ...
            if (metadata->has_flags(RECORD_FLAG, POINTER_FLAG)) {
                ...
                DEBUG_LOG("\tPointerfull object");
                auto **pointers = reinterpret_cast<uint8_t **>(metadata + 1);
                for (; reinterpret_cast<uint8_t *>(pointers) < scan_ptr; pointers++) {
                    relocate(*pointers);
                }
            } else {
                DEBUG_LOG("\tPointerless object");
            }
        }
    }
}
...
void GarbageCollector::update_finalizers(GcSpace &oldSpace) {
    ...
    while (iter != end) {
        ...
        if (finalizer.object_metadata->is_forwarded()) {
            DEBUG_LOG("Updating reference from " << (void *) finalizer.object_metadata << " to "
                                                 << (void *) finalizer.object_metadata->forwarded_pointer);
        ...
        if (finalizer.is_in_space(oldSpace)) {
            DEBUG_LOG("Object abandoned in old space, finalizing");
...
GcSpace::GcSpace(size_t size) {
    ...
    DEBUG_LOG("Creating new gc space: " << (void *) start << " to " << (void *) end);
}

GcSpace::GcSpace(GcSpace &&space) noexcept {
    DEBUG_LOG("Moving gc space to other variable");
...
GcSpace::~GcSpace() {
    DEBUG_LOG("Deleting gc space: " << (void *) start);
    delete[] start;
}
...
GarbageCollector garbageCollector = GarbageCollector(1024 * 1024);